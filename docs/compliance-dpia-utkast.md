...
Systemet skal behandle helseopplysninger (testresultater, psykologiske skårer, journalrelatert informasjon) om pasienter hos en privatpraktiserende autorisert psykologspesialist og kontorfellesskapets øvrige behandlere. Produksjonsdata lagres i en administrert skyløsning (Azure, Norway-region — se `beslutningslogg.md`), ikke lenger på egen server. Lokalt utviklingsmiljø bruker kun syntetiske/fiktive data.
...
**Tilgangsstyring:**
- Rollebasert tilgangskontroll (admin/utvikler, behandler, pasient), med prinsippet om minste privilegium — en behandler skal kun se egne pasienter, ikke andre behandleres.
...
- Fullstendig audit-logg over hvem som har åpnet/endret hvilke pasientdata, når — kjernekrav i Normen. Bygges inn i kodearkitekturen fra fase 1, aktiv i alle miljøer inkludert lokal utvikling.
...
**Driftssikkerhet:**
- Administrert patching/herding følger med Azure App Service og Azure Database for MySQL som plattformtjenester.
- Egne, isolerte ressursgrupper for dev/test vs. produksjon i Azure.
- Application Insights for strukturert logging/observability fra dag én.
...
- Skriftlig databehandleravtale (DPA) må inngås med enhver tredjepart som får tilgang til eller lagrer helseopplysninger på vegne av virksomheten: Microsoft/Azure, SMS-/e-postleverandør (hvis de ser innhold), BankID-leverandør, og Vipps. Kartlegges konkret når hver leverandør velges (fase 1 for Azure, fase 2 og 6 for de øvrige).

## 5. Lagringstid

Automatisk sletting av pasientdata etter 10 år, som stemmer med den generelle normen for oppbevaring av pasientjournaler i privat praksis (minst 10 år etter siste journalføring, jf. Psykologforeningens veiledning). Arkiverte (men ikke slettede) pasienter bør ha samme sikkerhetsnivå som aktive.
...
| Databehandleravtale med Microsoft/Azure ikke gjennomgått ennå | Middels | Gjennomgå Microsofts standard DPA og datalagringsvilkår før produksjonsdata legges inn |
| Ingen dedikert internkontrollrutine/avvikssystem beskrevet ennå | Middels | Etabler enkel internkontroll (dokumentasjon, avvikslogg) parallelt med teknisk utvikling |
| ~~Egen Windows-server som eneste driftsmiljø~~ | Løst | Produksjon flyttet til administrert skyløsning (Azure) |
...
2. Utviklings- og testmiljø (fase 1) bruker kun syntetiske/fiktive data — ingen ekte pasientdata før DPIA er sluttført.
3. Endelig teknisk sikkerhetsdesign (kryptering, logging, tilgangsstyring) spesifiseres i detalj i fase 2–4, med dette dokumentet som rammeverk.
